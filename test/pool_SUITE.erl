...
hello_pool_h1(Config) ->
	...
	do_wait_pool_up(ManagerPid),
	%% @todo Should pool functions return ok|error tuples?
	Streams = [gun_pool:get("/",
...
max_streams_h1(Config) ->
	...
	do_wait_pool_up(ManagerPid),
	%% @todo Should pool functions return ok|error tuples?
	{_,_} = gun_pool:get("/delay", #{<<"host">> => ["localhost:", integer_to_binary(Port)]}),
	timer:sleep(500),
	error = gun_pool:get("/delay", #{<<"host">> => ["localhost:", integer_to_binary(Port)]}).
...
reconnect_h1(_) ->
	...
	Port = ranch:get_port(?FUNCTION_NAME),
	{ok, ManagerPid} = gun_pool:start_pool("localhost", Port, #{protocols => [http]}),
	do_wait_pool_up(ManagerPid),
	%% @todo Should pool functions return ok|error tuples?
	Streams1 = [gun_pool:get("/", #{<<"host">> => ["localhost:", integer_to_binary(Port)]})
		|| _ <- lists:seq(1, 8)],
	_ = [begin
		{response, nofin, 200, _} = gun_pool:await(StreamRef),
		{ok, <<"Hello world!">>} = gun_pool:await_body(StreamRef)
...
%		{degraded, _} = gun_pool:info(ManagerPid),
	do_wait_pool_up(ManagerPid),
	Streams2 = [gun_pool:get("/", #{<<"host">> => ["localhost:", integer_to_binary(Port)]})
		|| _ <- lists:seq(1, 8)],
	_ = [begin
		{response, nofin, 200, _} = gun_pool:await(StreamRef),
		{ok, <<"Hello world!">>} = gun_pool:await_body(StreamRef)
...
do_wait_pool_up(ManagerPid) ->
	timer:sleep(1),
	case gun_pool:info(ManagerPid) of
		{operational, _} ->
			ok;
		_ ->
			do_wait_pool_up(ManagerPid)
